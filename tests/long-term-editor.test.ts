...
test("controlled memory update logs decision and persists revised memory", async () => {
  ...
  try {
    ...
    assert.ok(logs.some((entry) => entry.includes("[LongTermMemoryEditor] Decision: replace")));
    assert.ok(logs.some((entry) => entry.includes("[LongTermMemoryEditor] Save Success")));
  } finally {
