...
export function Sidebar({
  ...
  return (
    <aside className="relative w-80 border-r border-white/10 bg-gradient-to-b from-white/[0.03] via-zinc-950/40 to-zinc-950/80 p-4 sm:p-5">
      ...
      <div className="relative flex h-full flex-col">
        <div className="mb-5 rounded-3xl border border-white/10 bg-white/[0.035] p-4 shadow-[0_18px_50px_rgba(0,0,0,0.25)] backdrop-blur-sm">
          ...
          <button
            className="inline-flex w-full items-center justify-center rounded-2xl border border-white/10 bg-gradient-to-r from-sky-500 to-emerald-500 px-3 py-2.5 text-xs font-semibold text-white shadow-[0_12px_28px_rgba(16,185,129,0.25)] transition hover:brightness-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400"
                ...
                <div className="flex items-start justify-between gap-3 px-1 py-1">
                  <button className="flex-1 text-left" onClick={() => onSelectActor(actor.id)}>
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-sm font-semibold tracking-tight ${activeActor ? "text-white" : "text-zinc-200"}`}>
                        {actor.name}
                      </span>
                    </div>
                  ...
                  <div className="flex flex-col items-end gap-1.5 opacity-90 transition group-hover:opacity-100">
                    ...
                    <button
                      className="rounded-xl border border-red-500/20 bg-red-500/5 px-2.5 py-1 text-[11px] font-medium text-red-200 transition hover:border-red-400/40 hover:bg-red-500/10 hover:text-white"
